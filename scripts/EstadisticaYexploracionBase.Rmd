...
hist(log10(datos.sinhojas$long.hoja.cm.))
```
...
sum(complete.cases(datos.long$peciolo.prom.lam.prom)) #363
sum(complete.cases(datos.long$min.max.prom)) #358
sum(complete.cases(datos.long$long.hoja.cm.)) #234
sum(complete.cases(datos.long)) #How many sp have the three values: 61
#Chcek how many species have one of the three values
sum(complete.cases(datos.long))
sum(rowSums(!is.na(datos.long))>0) #564 tienen al menos un datos de longitud de hoja. 
